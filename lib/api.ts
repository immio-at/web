...
// 'scraped' carries the public `Listing` row. 'own' additionally carries the
// user's `UserListing` (funnel-state info — status, analysis/doc counts) so
// the dashboard renders it via ownPropertyToCard directly, with no client-side
// sourceUrl join back to the funnel cache.
export type RecentlyViewedItem =
  | { kind: 'own'; lastAt: string; listing: Listing; userListing: UserListing }
