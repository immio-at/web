...
/**
 * SmartSearchFieldImpl — ADR-024 §1 / §6. The pill-bar smart-search field.
 *
 * This is the *heavy* half — it pulls in the tokeniser, Fuse.js and the
 * bilingual vocabulary. It is `next/dynamic`-imported by `SmartSearchField`
 * only once the user focuses the field (ADR-024 §8.3), so the smart-search
 * chunk stays out of the Discover route's first-load JS.
 *
 ...
 * via `onApply`. After an apply, the matched token(s) are stripped from
 * the draft (§6.2). `⌘K` / `Ctrl+K` focuses the field; Escape dismisses
 * the dropdown then blurs.
 */
...
interface Props {
  ...
  /** Focus the input on mount — set by the lazy wrapper after activation
   *  so the focus that triggered the load carries through to this field. */
  autoFocus?: boolean;
}
...
export default function SmartSearchFieldImpl({ onApply, autoFocus = false }: Props) {
  ...
  // Carry the activating focus through from the lazy wrapper's resting
  // shell — focusing the input fires its `onFocus`, which opens the field.
  useEffect(() => {
