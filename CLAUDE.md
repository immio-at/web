...
- **Analyse button on every PropertyCard.** New inlined lucide-style magnifying-glass SVG (no `lucide-react` dep) rendered right of the property title via a `flex items-start gap-2` container that wraps the existing `<a>` (with `flex-1 min-w-0`) and the new `<button type="button">`. Click fires the same `actions.onAnalyse?.(item)` as image-tap — same modal entry point, more discoverable affordance. Colour: `text-gray-400 hover:text-gray-600` when both `analysisCount` and `documentCount` are 0; `text-blue-600 hover:text-blue-700` once either is > 0. `Property` interface (`lib/api.ts`) + `CardProperty` interface (`PropertyCard.tsx`) gain optional `analysisCount?` + `documentCount?`. **All five card-mapper sites updated** — Dashboard `ownPropertyToCard`, search/page `propertyToUnified` + `listingToCard`, FinderClient `propertyToCard` + `finderCardToCardProperty`, FunnelBoard `propertyToCard`, RecommendedCarousel `propertyToCard`. Scraped tiles default to 0 (no backing Property yet); search/page's `listingToCard` does a `cachedProperties.find(p => p.sourceUrl === l.sourceUrl)` lookup so a scraped tile that maps to an existing own Property surfaces the upgraded counts (heart-saved-then-engaged scraped listings still show blue). The button uses `e.preventDefault(); e.stopPropagation();` so it never triggers the parent `<a>` link.
