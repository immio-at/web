...
- Layout: image on top with three overlays — source badge top-left (green "… Suchagent" for email-parsed, grey for scraped), heart top-right, vertical action stack right-middle (hover-visible on desktop, always-visible on mobile): **external-link / ⚠ / ✕** (post-ADR-012 v1.1; the v1.0 `🔍` view button was replaced by the external-link icon). Expired badge bottom-left.
- **Image-tap (ADR-012 v1.1 PC5)** — image is a `<button>` that opens `PropertyAnalysisModal` via `actions.onAnalyse?.(item)`. 6px movement-threshold guard plus `stopPropagation()` so Finder swipes (parent pointer-events) and Funnel drags (HTML5 `draggable`) don't fire accidental taps. The card-title `<a>` link in the body retains its source-URL behaviour.
