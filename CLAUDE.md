...
└── (authenticated)/                 ← Route group — all protected pages
    ├── layout.tsx                   ← Wraps all authenticated pages with NavBar, SSEProvider, FeedbackButton, OnboardingTour, AdminUnacknowledgedToast (admin-only, self-gating)
    ...
    ├── help/page.tsx                ← Help Hub (ADR-021, Session 50) — Tour restart, Starting from Scratch, Email Forwarding, Feedback
    ├── inbox/page.tsx               ← Inbox (ADR-020 v1.1, Session 50) — allowlist-captured emails (Gmail confirmations, parser failures)
    ├── reports/page.tsx             ← Feedback Reports admin view (ADR-018 §5, moved 2026-05-14 from /admin/reports) — admin-only, self-gates on isAdmin
    └── admin/page.tsx               ← User management, invite codes, scraper health (NOT reports — see /reports above)
```
...
- `optimisticUpdate()` patches arbitrary `UserListing` fields in cache instantly
...
**ADR-025 M2b — unified-schema type names (Phase 2, frontend types refactor):**
the funnel-entity interface `Property` was renamed `UserListing`, the public
listing entity `ScrapedListing` → `Listing` (gains `visibility: 'public'|'private'`),
and `PropertyAnalysis` → `Analysis`. The shared card view-model is now
...
2. **Track 4 P4 — Dark mode** — OS `prefers-color-scheme` default + Light/Dark/Auto override in Settings.
3. **Track 5 — Apple + LinkedIn Sign-In** — prerequisites for native mobile app.
4. **Track 6 PL2 — Native mobile app** — React Native / Expo, iOS + Android.
5. **Track 8 — Onboarding wizard, map view** — deferred until tester feedback.

*Track 4 P1 (Email forwarding setup assistant) superseded by Track 22 / ADR-021 — forwarding setup lives on the Help page.*

Recently completed (Session 55, 2026-05-19):
- **Performance pass — Discover, Dashboard, Analytics load time.** No ADR; a load-time pass run as four workstreams, all shipped to dev + main.
- **Discover smart-search lazy-loaded.** `SmartSearchField.tsx` is now a lightweight wrapper that renders only the resting chrome (`RestingShell`); the heavy implementation — tokeniser, Fuse.js, bilingual vocabulary — lives in the new `SmartSearchFieldImpl.tsx` and is `next/dynamic`-imported (`ssr: false`) only on first focus / `⌘K`. The smart-search chunk leaves Discover's first-load JS for users who never search. `SmartSearchFieldImpl` takes an `autoFocus` prop so the activating focus carries through to the real input.
- **`⌘K` hint made platform-aware.** New `components/filters/useShortcutLabel.ts` hook — post-mount `navigator` sniff shows `⌘K` on macOS, the localised Ctrl label otherwise (`smartSearch.shortcutKey` — "Strg K" / "Ctrl K"). Both `SmartSearchField`'s `RestingShell` and `SmartSearchFieldImpl` render `{shortcut}` instead of a hardcoded `⌘K`. The shortcut itself still works everywhere (listens for `metaKey || ctrlKey`).
- **Dashboard no longer blocks behind `/properties` (W1).** `dashboard/page.tsx` stopped early-returning a full-page skeleton gated on `useProperties().loading`. It always renders `DashboardClient` now, passing `propertiesLoading`. The summary-tile grid mounts immediately so the self-fetching tiles (`AnalyticsSnapshotTile` → `getAnalyticsSummary`, `DashboardAnalysisTile` → `usePortfolioAnalyses`) start their requests in parallel with `/properties` instead of behind it. The carousels genuinely need the property list, so they self-skeleton via a `CarouselSkeleton` in `DashboardClient`; the HH9 empty-state banner and the New Arrivals scraped-fallback fetch are both gated on `!propertiesLoading` so they don't flash / fire a wasted request before the real list arrives.
- **`AuthContext` prefetch fixed (W7).** `prefetchProperties` / `prefetchSavedFilters` were checking the `session` *state* in the mount effect body — `null` on a cold load before `getSession()` resolves, so the prefetch never ran. Moved into the `getSession()` callback (cold page-load) **and** the `onAuthStateChange` listener (same-tab sign-in — that path never re-runs the mount effect). Both prefetch fns are TTL-guarded, so token-refresh firings are no-ops.
- **`DiscoverTile` scraped-count fan-out deferred.** The tile fires 10 `getScrapedListings` queries on mount (1 global + 9 per-Bundesland) to pre-cache state-pill counts. Once W1 mounted all tiles immediately, those 10 full-table queries began racing `/properties` for the 5-connection pgbouncer pool — ~13s to paint the carousels on a fresh sign-in. The fan-out now runs via `requestIdleCallback` (3s hard cap, `setTimeout` fallback) so the carousels paint first; the counts only feed the toggle badge and were never critical-path. Tracked as TD24 — the proper fix is a single backend per-state count endpoint.
- **recharts split out of the modal chunk (W4).** The two analysis charts moved to `components/analysis/AnalysisCharts.tsx` (`OwnerWealthChart`, `RentalProjectionChart`); `PropertyAnalysisModal` `next/dynamic`-imports them. recharts is heavy and `PropertyAnalysisModal` is itself lazy, but a dossier-mode open — the common case — never renders a chart, so it no longer downloads recharts at all.
- **Analytics tile refreshes on SSE (W3).** The SSE `analytics` event called `clearAnalyticsCache()` which nulled the module cache but never re-rendered anything — the tile only refetched on mount / userId change. `AnalyticsSnapshotTile` now exports `refreshAnalyticsFromServer()` (refetch + push to a listener set) and registers each mounted tile as a listener; `useSSEInvalidation` calls it instead of `clearAnalyticsCache()`. `clearAnalyticsCache()` stays for sign-out.

Recently completed (Session 54, 2026-05-18):
- **ADR-024 Smart Search Field (Track 29) — C1–C3 shipped behind `SMART_SEARCH_ENABLED = false`.** A pill-bar R3 text field that tokenises typed Austrian property vocabulary into chip / slider / postcode **suggestions**; it routes *into* the pill bar, never replaces it. **C1** — new `lib/smart-search/` pure library: `normalise.ts` (ß/umlaut folding), `tokens.ts` (bilingual DE+EN+abbreviation maps for property type / rent regulation / Bundesland + noise words), `vienna-districts.ts` (23 districts → postcodes + display names), `numeric.ts` (regex range parser), `types.ts`, `tokeniser.ts` (greedy n-gram window, ambiguity expansion, Fuse.js fuzzy fallback `FUZZY_THRESHOLD = 0.3`). **`vitest` was added to the repo** (no test runner existed before) — `npm test` runs `tokeniser.test.ts` + `numeric.test.ts`, 43 cases. **C2** — `components/filters/SmartSearchField.tsx` (debounced 150ms tokenise, `⌘K`/`Ctrl+K` focus, rotating placeholder, Escape→close→blur, click-outside), `SmartSearchDropdown.tsx`, `SmartSearchSuggestion.tsx`. `PresetFilters` gained a `showSmartSearch` prop and `applySuggestion(s)` — a router that maps a suggestion's structured `target` (a discriminated union) into the pill bar's chip / preset-key / range-slider / `keyword` state. The R3 slot renders only when `pillBarLive && showSmartSearch`; it collapses with no gap otherwise. `config/feature-flags.ts` gained `SMART_SEARCH_ENABLED` (default `false` — flip at C5); `search/page.tsx` passes it as `showSmartSearch`. New `smartSearch.*` i18n namespace in `messages/{de,en}.json`. **The tokeniser is pure and decoupled** — it never imports `FilterValues`; suggestions carry a structured `target` the component applies. **Discover-only** — Funnel / Finder / Dashboard render `PresetFilters` without `showSmartSearch`. **C5 shipped** — `SMART_SEARCH_ENABLED = true`; `components/filters/SmartSearchToast.tsx` is a one-time first-visit "what's new" toast (per-user-id `immio.smartSearchToast.seen:{userId}` localStorage flag — same pattern as the tour-completion flag; dismissible, auto-hide 12s) mounted on `search/page.tsx`. The flag stays reversible. C4's formal Anna walkthrough + `FUZZY_THRESHOLD` tuning can run post-release. See ADR-024.
- **Filter pill-bar layout reconciled with ADR-023 §1.1.** A Session 53 ad-hoc tweak had drifted from the ADR; `PresetFilters.tsx` is now back on the §1.1 row order — R4 type → R5 regulation → **R6 sliders → R7 TOM** → R8 sort → **R9 postcode** (its own row, not merged into the R1 Bundesland row). The four range sliders render **two per row** (Price + €/m², then Size + Rooms) per the new ADR-023 §2.6. Each pair is a `flex flex-wrap` row of two `flex-1 min-w-[200px]` sliders.
- **ADR-023 v1.3 §10.5 — pill-bar visual hierarchy.** Ten visual-hierarchy rules implemented. R1 and R9 are now **multi-select dropdowns** — new `components/filters/StateDropdown.tsx` (9 Bundesländer, checkbox panel) and `PostcodeDropdown.tsx` (checkbox panel + numeric typeahead over all Austrian postcodes via the new `getAllPostcodes()` in `austria-plz-bundesland.ts`); the free-text `PostcodeEntry.tsx` is **deleted** and the inline `StatePill` removed. `UserFilterPill` carries a bookmark-icon prefix. `RangeSlider` has an at-rest treatment — `isAtRest = lo <= min && hi >= max` suppresses the fill, shrinks the handles to 12px with a grey border, and the label-row readout shows "Alle"/"All" (vs the "min – max" range when touched); the label stays full weight in both states. `PresetFilters` renders small-grey row labels via a shared `rowLabelClass`, hosts `MoreFiltersPrompt` inside R2 next to "Create new filter" (the standalone R10 row is gone), and groups rows with `groupSpacing` vs `innerSpacing`. **Row order** (ADR-023 §1.1, settled): R1 smart search → R2 "Your filters" (`savedFiltersRow`, directly below the search bar) → **R3 Location** (`locationRow` — State + Postcode dropdowns side by side) → R4 source (`sourceRow`) → R5 type → R6 regulation → R7 TOM → R8 sliders → R9 sort. The TOM chip row sits above the sliders so the pill rows cluster.
- **Filter box + sort placement (ADR-023 §10.5.8).** `PresetFilters` wraps the bar in a bordered card on the full-size surfaces (`compact ? bar : <div box>{bar}</div>` — Discover, Funnel); compact tile / Finder stay un-boxed. `PresetFilters` takes a `showSort` prop (default true); `search/page.tsx` passes `showSort={false}` and renders its own `<SortDropdown bare />` next to the "X listings found" count with a divider. `SortDropdown` `bare` prop drops the fixed-width label column for that standalone placement.
- **Other filter polish.** Slider units live in each slider label ("Preis (€)" / "Größe (m²)" / "Preis (€/m²)" / "Zimmer") — the `unit` prop is no longer passed. Chip-row labels dropped "Property" → "Type" / "Classification" / "Source" ("Typ" / "Klassifizierung" / "Quelle"). Row labels use a fixed-width column (`rowLabelClass`, `w-28` / compact `w-24`) so every row's controls left-align. "Clear all" now also resets the pill-bar `values` (postcode/sliders/chips/TOM), gated by `isFilterActive(values)` so it shows when only a values-based filter is set. New `presetFilters.*` i18n added across these changes (`rangeAll`, `yourFiltersLabel`, `sourceLabel`, `locationLabel`, `stateDropdownPlaceholder`, `postcodeDropdownPlaceholder`, `postcodeSearchPlaceholder`, `postcodeSearchHint`, `moreCount`).

Recently completed (Session 51 iteration, 2026-05-14 → 2026-05-15):
- **Price inputs step by thousands.** Default `<input type="number">` step is 1 — useless on 5–7-figure property prices. Filter forms (`DiscoverTile`, `FilterBar`, `FilterModalForm`) got `step="1000"` on `minPrice` / `maxPrice` and `step="100"` on `minPricePerSqm` / `maxPricePerSqm` (€/m² is typically 1000–15000 in AT — hundreds is the right grain). PropertyAnalysisModal's `NumInput` component gained an optional `step?: number | string` prop (default `'any'` to preserve existing behaviour for percentages and small fields); `step={1000}` passed on the editable property-scale € fields: `desiredPrice`, `otherPurchaseCosts`, `loan1Amount`, `loan2Amount`, `flipResalePrice`.
- **FilterBar layout reflow.** With PT1 hiding the keyword input, Line 1 had become just the Location bar by itself (wide and visually heavy). Moved Location onto Line 2 alongside Price + €/m² with `flex-1` to absorb leftover width. Line 1 stays in the JSX but only renders when `SEARCH_INPUT_ENABLED` flips back to `true`.
- **Reports admin team-note typing race fixed.** Reported as "characters appear, disappear and reappear" when typing in the `/reports` team-note textarea. Root cause: the row's `useEffect([report.teamNote])` resynced local state from props after every save round-trip, clobbering characters typed during the ~100ms server echo. Final fix: dropped the resync `useEffect` entirely. Local `note` state is seeded once from props via `useState`'s initializer and is the sole source of truth from row mount until unmount. Rows have `key={report.id}` so filter changes / refreshes unmount and remount with fresh seed values. Trade-off: out-of-band updates (another admin editing the same row in another tab) won't propagate to a currently-mounted row until refresh — acceptable for the tester window.
- **Funnel heart-click stage picker: Not Relevant option added.** The kanban heart dropdown populated from `FUNNEL_STAGES_DISPLAY` (which intentionally excludes `'new'` and `'not_relevant'` — neither is a kanban column). Left users with no in-dropdown path to Not Relevant. Appended a `not_relevant` option with `variant: 'danger'` (red text + divider above) after the regular stage options. `requestMove()` already routes `'not_relevant'` through the irreversible-action confirm modal, so no new safety scaffolding needed.
- **PropertyCard bottom-right buttons → true circles.** Full-mode `actionBtn` used `p-1.5` (padding-based sizing) so the rendered width depended on the inner glyph: ⚠ and ✕ were wider than the `ExternalLinkIcon` SVG, giving oval/pill shapes. Switched full-mode to `w-8 h-8` to match the top-right cluster's fixed square dimensions. All four card icons now render as identically-sized circles.

Recently completed (Session 50 iteration, 2026-05-14):
- **Per-user-id tour-completion flag.** The original unscoped key `immio.onboardingTour.completed` was wiped in `clearAllUserCaches()` on every sign-out to defend cross-user separation, which regressed same-user sign-out/sign-in (tour re-fired). Switched to keyed `immio.onboardingTour.completed:{userId}` — cross-user separation is structural so the wipe is unnecessary. The `TOUR_COMPLETED_KEY` export is gone; `tourKey(userId)`, `readCompleted(userId)`, `writeCompleted(userId, value)` are internal. `restartOnboardingTour(userId)` now takes the userId; `HelpTourRestartSection` pulls it from `useAuth()`. `AuthContext` `clearAllUserCaches` drops the tour-flag removal with a comment explaining why.
- **Help moved to utility area.** ADR-021 intent was an icon button alongside Settings, not a labelled product-feature nav entry. Removed Help from `navItems`; added `HELP_NAV` as a `?` icon button styled to match the `⚙` Settings cog, slotted between the language toggle and Settings. Mobile keeps the labelled link in the nav row (no utility area on small viewports — uses `[...navItems, HELP_NAV].map`). `data-tour-id="nav-help"` preserved on the desktop button; `nav-help-mobile` preserved on the mobile row.
- **Tour overlay teardown on /dashboard exit.** Reported as "clicking properties on Discover doesn't open the modal." Root cause: the first-run gate only checked `pathname === '/dashboard'` to START the tour; once `run=true` Joyride stayed mounted across route changes and its full-viewport overlay swallowed clicks on /search, /funnel, etc. Added a second effect that sets `run=false` when pathname leaves /dashboard, plus a render guard (`if (!run || pathname !== '/dashboard') return null`) so the overlay never appears on non-dashboard routes even if state is briefly stale during a route change. Doesn't mark complete — navigation is a tacit dismiss; next /dashboard visit re-fires.
- **Scraped image-tap opens the full analysis modal.** Reported as "the modal isn't loading on Discover" after the overlay fix. Root cause: `onAnalyse` for scraped items only logged a `view` interaction and silently did nothing (a deliberate ADR-012 v1.1 decision on the grounds that "scraped has no analysis," but a new user with zero own properties saw every Discover click as dead). Decision reversed at user direction: "nothing different between scraped and forwarded other than their source." `saveScrapedListing(id, status?)` in `lib/api.ts` now sends a JSON body with the status; default `'investigating'` preserves heart-click. Discover (`search/page.tsx`), Dashboard (`DashboardClient.tsx`), Finder (`FinderClient.tsx`) `onAnalyse` for scraped now: (a) finds an existing Property by `sourceUrl` and opens the modal on it, or (b) calls `saveScrapedListing(id, 'new')`, `optimisticInsert(property)`, then opens the modal on the freshly-created Property. Funnel kanban hides `'new'` so casual clicks don't pollute the active funnel; the row stays visible on Discover via the existing own@new filter. Heart-click behavior unchanged (still creates at `'investigating'` via the same endpoint with the default body). Finder additionally refactored — `showAnalyseModal` boolean collapsed into `analyseProperty: Property | null` so the modal mount no longer hard-gates on the deck card being own; new shared `openAnalyseFor(card)` helper drives both the swipe-down path (`handleAction('analyse')`) and the image-tap `onAnalyse` path. ADR-012 v1.2 in spirit becomes "image-tap = open analysis modal" universally; the source distinction now affects only persistence semantics (the new Property's status), not UI behavior.
- **Tour copy refinements + 8th step (Feedback).** Step 6 said "heart icon" — the actual card icon is a house (ADR-012 v1.2 swapped heart → house and the tour copy was never updated); fixed in DE + EN. DE step 4 "triagieren" → "priorisieren" per copy review. New step 7 highlights the floating Feedback button — copy "use this button to report problems or request improvements" / "Probleme melden oder Verbesserungen vorschlagen"; new `data-tour-id="feedback-button"` on `components/feedback/FeedbackButton.tsx`. Old step 7 (Help nav) becomes step 8. Step 4 (Finder) extended with up/down swipe documentation: "right to investigate further in your Funnel, left to dismiss, **up to view the listing, down to analyse the property**" / "Rechts: ... Links: ... **Hoch: Inserat ansehen. Runter: Immobilie analysieren.**" Tour now eight steps. `data-tour-id` convention list in this file updated with `feedback-button`.
- **Analyse button on every PropertyCard.** New inlined lucide-style magnifying-glass SVG (no `lucide-react` dep) rendered right of the property title via a `flex items-start gap-2` container that wraps the existing `<a>` (with `flex-1 min-w-0`) and the new `<button type="button">`. Click fires the same `actions.onAnalyse?.(item)` as image-tap — same modal entry point, more discoverable affordance. Colour: `text-gray-400 hover:text-gray-600` when both `analysisCount` and `documentCount` are 0; `text-blue-600 hover:text-blue-700` once either is > 0. `Property` interface (`lib/api.ts`) + `ListingCard` interface (`PropertyCard.tsx`) gain optional `analysisCount?` + `documentCount?`. **All five card-mapper sites updated** — Dashboard `ownPropertyToCard`, search/page `propertyToUnified` + `listingToCard`, FinderClient `propertyToCard` + `finderCardToListingCard`, FunnelBoard `propertyToCard`, RecommendedCarousel `propertyToCard`. Scraped tiles default to 0 (no backing Property yet); search/page's `listingToCard` does a `cachedProperties.find(p => p.sourceUrl === l.sourceUrl)` lookup so a scraped tile that maps to an existing own Property surfaces the upgraded counts (heart-saved-then-engaged scraped listings still show blue). The button uses `e.preventDefault(); e.stopPropagation();` so it never triggers the parent `<a>` link.
- **Dashboard Enter-runs-search fix.** Reported as "Enter on Dashboard opens the Add Property modal." Root cause: default `<button>` is `type="submit"`; `AddPropertyButton` had no explicit type, so any implicit-submission scenario could intercept Enter on a sibling input. Two changes: (a) `AddPropertyButton` now declares `type="button"` explicitly — defensive everywhere it's used, not just on Dashboard. (b) `DiscoverTile` imports `useRouter` from `@/i18n/navigation` and wraps its filter inputs in a `<form onSubmit={(e) => { e.preventDefault(); router.push('/search' + buildQueryString()); }}>` so Enter in any input runs the search exactly like clicking the Search button. A hidden `<button type="submit" hidden tabIndex={-1} aria-hidden />` inside the form makes Enter-to-submit work without changing the visible action row — the Search `<Link>` stays outside the form so Cmd-click still opens in a new tab.
- **/admin/reports → /reports + AdminUnacknowledgedToast moves global.** Author direction: split reports from admin (different concerns), give Reports its own utility-area nav entry between Admin and Sign Out, and elevate the unacknowledged banner to the top of every page for admins so they don't have to navigate to see it. Three changes: (a) page file moves verbatim from `app/[locale]/(authenticated)/admin/reports/page.tsx` to `app/[locale]/(authenticated)/reports/page.tsx` as its own top-level admin-only route. `/admin` (users / invite codes / scraper health) is unchanged. `/admin/layout.tsx` is deleted (only existed to host the toast). (b) `AdminUnacknowledgedToast` moves from `/admin/layout.tsx` to the global `(authenticated)/layout.tsx` so the amber banner renders at the top of every authenticated page for admins. Self-gates on `isAdmin && count > 0` so it's a no-op for non-admins and admins-with-zero-unread (no chrome leak). Toast href updated to `/reports?unacknowledged=true`. (c) `NavBar` gains a "Reports" / "Berichte" link in the utility area, slotted between Admin and Sign Out (only rendered when `isAdmin`), with the same amber chrome as the Admin link. Mobile nav row gets the same labelled link with a 📨 icon. `nav.reports` i18n keys added DE + EN. ADR-018 §5.5 _As Implemented_ amendment: the toast is no longer scoped to `/admin/*` — global mount + self-gate is structurally equivalent for non-admins and gives admins better situational awareness.

Recently completed (Session 50, 2026-05-12 → 2026-05-13):
- **Track 22 (ADR-021) Help & Onboarding Hub — shipped end-to-end.** New `/help` route (`app/[locale]/(authenticated)/help/page.tsx`) with four section components in `components/help/`: `HelpTourRestartSection`, `HelpGettingStartedSection`, `HelpForwardingSection` (Gmail / Outlook / Apple Mail sub-sections + honest amber "Known gap" notes on Outlook + Apple per ADR-021 v1.2 — Outlook + Apple Mail confirmation senders aren't on the inbound allowlist yet so testers whose confirmation code doesn't appear in /inbox should contact via Feedback), `HelpFeedbackSection`. `OnboardingTour.tsx` mounted at the authenticated layout (`(authenticated)/layout.tsx`) — uses `react-joyride@3` v3 API (`onEvent` + `EVENTS.TOUR_END`, `options.buttons = ['back', 'skip', 'primary']`, `options.overlayClickAction = false`, teal primary `#0d9488`). First-run gate: localStorage flag `immio.onboardingTour.completed` absent AND `pathname === '/dashboard'` AND `window.innerWidth >= 600` AND no `[data-property-modal-open]` element; 800ms start delay so dashboard data settles + modal-open predicate is re-checked at fire time. Seven steps in `help.tour.steps.*` (1 centred welcome, 2 Sources tile, 3 Funnel nav, 4 Finder nav with "Finder — Tinder für Immo" / "Finder — Tinder for Real Estate" tagline matching the landing-page hero, 5 Discover nav, 6 first card across the Dashboard carousels, 7 Help nav). `restartOnboardingTour()` exported for the Help page's "Restart tour" button — wipes the flag and routes to `/dashboard`. **`data-tour-id` convention** documented in this file — preserved across refactors; current targets are `dashboard-sources-tile`, `nav-funnel`/`nav-finder`/`nav-discover`/`nav-help` (desktop + `*-mobile` siblings), and `dashboard-first-card` (propagated to every carousel's first card via new `firstCardTourId` prop on `PropertyCarousel` + `RecommendedCarousel` so `document.querySelector` picks the topmost non-empty in document order without coordination state). **HH9 Dashboard empty-state banner** in `DashboardClient` — renders when `properties.length === 0` with a teal "Set up email forwarding →" link to `/help#forwarding`. **Sign-out** clears `TOUR_COMPLETED_KEY` in `clearAllUserCaches` (so a second user on the same browser still sees the tour). New Help nav entry in `NavBar` between Analytics and Settings (`nav.help` i18n key) — `data-tour-id` plumbed on both desktop + mobile rows. Full DE + EN i18n under `help.*` (tour copy + four section copies + Gmail/Outlook/Apple step lists with `1`..`8` keys for Gmail, `1`..`6` for Outlook + Apple).
- **Track 23 (ADR-020 v1.1) Inbox surface (II8) — shipped end-to-end.** New `app/[locale]/(authenticated)/inbox/page.tsx` mounts `InboxList` (paginated 25/page, "Load more" button). `InboxRow` is expand-in-place — first expansion fires `PATCH /inbox/:id/read` silently (failure is non-blocking; SSE catches up next cycle) + flips a teal dot to transparent + bolds-to-normal. Body rendering prefers `bodyText`; falls back to `bodyHtml` stripped to text (defensive — allowlist already constrains senders, but body is never `innerHTML`-injected); falls back to "(body unavailable)". `failureReason`-aware help line below the body: Gmail confirmation → "Copy the code below back into your Gmail settings"; unknown_platform → informational "no action needed"; parse_empty/error → "contact us via Feedback". `SourcesSetupTile` extended — fetches `getInboxUnreadCount()` on mount + on every `inbox` SSE cache-invalidation event, renders "1 unread system message — view" / "{count} unread system messages — view" link inside a top-bordered footer when count > 0 (absent when 0, keeping the tile clean in the steady state). `useSSEInvalidation` dispatches `window.CustomEvent('immio:cache-invalidation', { detail: { type: 'inbox' } })` for the `inbox` channel so listeners decouple from the hook. API client `lib/api.ts` gains `InboundEmail` / `InboxListResponse` types + `getInbox(page)`, `getInboxUnreadCount()`, `markInboxRead(id)` helpers. Full DE + EN i18n under `inbox.*` (pageTitle, pageDescription, empty/loading/loadMore/errorPrefix/noSubject/fromLabel/subjectLabel/bodyUnavailable/unreadIndicator, plus `senderCategory.{gmail_forwarding,willhaben,immoscout24,bazar,immmo,unknown}` labels and `failureReason.{gmail_confirmation,unknown_platform,parse_empty,parse_error}` help lines). Plus new `dashboard.sourcesTile.unreadSingle` + `unreadPlural` ICU strings and a new `dashboard.empty.{noPropertiesYet,setupForwardingLink}` namespace for HH9.
...
- Exports: `ListingCard` interface, `CardActions` interface.
...
- **Auth prefetch fires twice (Session 55):** `AuthContext` calls `prefetchProperties()` + `prefetchSavedFilters()` from BOTH the `getSession()` callback (cold page-load) AND the `onAuthStateChange` listener (same-tab sign-in — that path never re-runs the mount effect). Both fns are TTL-guarded so the token-refresh firings of the listener are no-ops. Do not move the prefetch back into the mount-effect body — the `session` *state* is `null` there on a cold load.
- Analytics summary: 5-minute module-level cache in AnalyticsSnapshotTile. SSE `analytics` events call `refreshAnalyticsFromServer()` (refetch + push to mounted tiles via a listener set) — NOT `clearAnalyticsCache()`, which only nulls the cache and is reserved for sign-out (Session 55, W3).
- **Dashboard mounts tiles before `/properties` (Session 55, W1):** `dashboard/page.tsx` does not gate on `useProperties().loading` — it renders `DashboardClient` immediately with a `propertiesLoading` prop. The self-fetching tiles (Analytics / Analysis) request in parallel with `/properties`; the carousels self-skeleton. **`DiscoverTile`'s 10-query per-state scraped-count fan-out is deferred to `requestIdleCallback`** so it doesn't saturate the 5-connection pgbouncer pool while the carousels are still loading (TD24).
...
- PropertyAnalysisModal: dynamic import (`next/dynamic`, ssr: false). recharts is split a second level deeper (Session 55, W4) — the two charts live in `components/analysis/AnalysisCharts.tsx` and are themselves `next/dynamic`-imported by the modal, so a dossier-mode open never downloads recharts.
